...
describe Pokemon do
  ...
  describe '#list_stats' do
    it 'shows a pokemons stats' do
      expect(pikachu.list_stats).to be_a(String)
    end
    it 'shows all stats' do
      expect(pikachu.list_stats.include?(pikachu.name)).to           eq(true)
      expect(pikachu.list_stats.include?(pikachu.id.to_s)).to        eq(true)
      expect(pikachu.list_stats.include?(pikachu.poketype.to_s)).to eq(true)
      expect(pikachu.list_stats.include?(pikachu.hp.to_s)).to        eq(true)
      expect(pikachu.list_stats.include?(pikachu.defense.to_s)).to   eq(true)
      expect(pikachu.list_stats.include?(pikachu.attack.to_s)).to    eq(true)
      expect(pikachu.list_stats.include?(pikachu.speed.to_s)).to     eq(true)
    end
  end
end
